        ...
        <tbody className="divide-y divide-slate-100">
          {scans.map((record) => (
            <tr key={record.id} className="align-top hover:bg-slate-50/70">
              ...
              <td data-label="Scan Type" className="px-5 py-4 text-slate-600">{record.scanType}</td>
              <td data-label="Scan Date" className="px-5 py-4 text-slate-600">{formatDate(record.scanDate)}</td>
              <td data-label="Next Scan Date" className="px-5 py-4 text-slate-600">{record.nextScanDate ? formatDate(record.nextScanDate) : "—"}</td>
              <td data-label="Health Centre" className="px-5 py-4 text-slate-600">{record.healthCentreName}</td>
