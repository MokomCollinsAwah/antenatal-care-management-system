"use server";

import { revalidatePath } from "next/cache";
import { z } from "zod";
import { getCareUser } from "@/features/appointments/queries";
import { getCurrentPortalPatientProfile } from "@/features/patients/queries";
import { ReminderStatus, UserRole } from "@/lib/constants";
import { getCurrentUser } from "@/lib/auth-utils";
import { updateReminderStatusSchema } from "@/lib/validations";
import { AdminServiceError } from "@/server/services/adminServiceError";
import { changePatientReminderStatus, changeReminderStatus } from "@/server/services/clinicalSupportService";
import type { ActionResult } from "@/types";

type Input = z.input<typeof updateReminderStatusSchema>;

export async function updateReminderStatusAction(input: Input): Promise<ActionResult<{ id: string }>> {
  const parsed = updateReminderStatusSchema.safeParse(input);
  if (!parsed.success) return validationFailure(parsed.error.flatten().fieldErrors);
  try {
    const data = await changeReminderStatus(parsed.data.reminderId, parsed.data.status as ReminderStatus, await getCareUser());
    revalidatePath("/reminders");
    return { success: true, message: "Reminder status updated successfully.", data };
  } catch (error) {
    return actionFailure(error, "Reminder action failed");
  }
}

export async function updateOwnReminderStatusAction(input: Input): Promise<ActionResult<{ id: string }>> {
  const parsed = updateReminderStatusSchema.safeParse(input);
  if (!parsed.success) return validationFailure(parsed.error.flatten().fieldErrors);
  try {
    const user = await getCurrentUser();
    if (!user || user.role !== UserRole.PREGNANT_WOMAN) {
      return { success: false, message: "You are not authorized to perform this action." };
    }
    const profile = await getCurrentPortalPatientProfile(user.id);
    if (!profile) return { success: false, message: "Patient not found." };
    const data = await changePatientReminderStatus(parsed.data.reminderId, parsed.data.status as ReminderStatus, profile.id, user.id);
    revalidatePath("/portal");
    return { success: true, message: "Reminder status updated successfully.", data };
  } catch (error) {
    return actionFailure(error, "Portal reminder action failed");
  }
}

function validationFailure(errors: Record<string, string[] | undefined>): ActionResult<never> {
  return { success: false, message: "Please correct the highlighted fields.", errors: Object.fromEntries(Object.entries(errors).filter((entry): entry is [string, string[]] => Boolean(entry[1]))) };
}

function actionFailure<T = never>(error: unknown, label: string): ActionResult<T> {
  if (error instanceof AdminServiceError) return { success: false, message: error.message };
  if (error instanceof Error && error.message === "Unauthorized") return { success: false, message: "You are not authorized to perform this action." };
  console.error(label, error);
  return { success: false, message: "The request could not be completed. Please try again." };
}
