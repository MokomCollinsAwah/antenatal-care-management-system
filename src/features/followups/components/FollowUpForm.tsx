    ...
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit as Parameters<typeof handleSubmit>[0])} noValidate>
      ...
      <FormSection title="Follow-up Information">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <Input label="Follow-up Date and Time" required type="datetime-local" error={errors.followUpDate?.message ?? result?.errors?.followUpDate?.[0]} {...register("followUpDate")} />
          <Select label="Method" required options={FOLLOW_UP_METHOD_OPTIONS} error={errors.method?.message ?? result?.errors?.method?.[0]} {...register("method")} />
          <Select label="Outcome" required options={FOLLOW_UP_OUTCOME_OPTIONS} error={errors.outcome?.message ?? result?.errors?.outcome?.[0]} {...register("outcome")} />
